...
const canClockOut=!!activeAtt;
 ...
 const tabs:[Tab,string][]=[['home','Ringkasan'],['announcements',announcementUnread>0?`Pengumuman (${announcementUnread})`:'Pengumuman'],['attendance','Absensi'],['leave',leavePending>0?`Cuti (${leavePending})`:'Cuti'],['overtime',overtimePending>0?`Lembur (${overtimePending})`:'Lembur'],['schedule','Jadwal'],['payslip',payslipUnread>0?`Slip Gaji (${payslipUnread})`:'Slip Gaji'],['feedback',feedbackUnread>0?`Kotak Saran (${feedbackUnread})`:'Kotak Saran'],['profile','Profil']];
...
function PortalLoadingScreen(){
 return <main className="employee-loading-screen" role="status" aria-live="polite">
  <section className="employee-loading-card">
   ...
   <div className="employee-loading-copy"><strong>Portal Karyawan</strong><span>Menyiapkan data Anda…</span></div>
