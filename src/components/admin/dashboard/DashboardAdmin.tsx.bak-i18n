...
function Overview({employees,attendance,present,late,payroll,onNavigate,profileName}:{employees:Karyawan[];attendance:Absensi[];present:number;late:number;payroll:number;onNavigate:(m:MenuKey)=>void;profileName:string}){
 ...
 return <div className="executive-dashboard">
  ...
  <div className="command-strip">
   <div><span className="eyebrow">STATUS OPERASIONAL</span><strong>Status sistem</strong><small>Data terhubung ke database</small></div>
  ...
  <div className="stat-grid executive-stats">
   ...
   <Stat title="Total Gaji Pokok" value={money(payroll)} hint="Dari master karyawan aktif" icon="payroll"/>
   <Stat title="Data Absensi" value={String(attendance.length)} hint="Data tersimpan" icon="clock"/>
  </div>
  <div className="dashboard-grid-top">
   ...
   <div className="panel attendance-health">
    <div className="panel-head"><div><span className="eyebrow">HARI INI</span><h2>Status Kehadiran</h2><p>Status kehadiran hari ini.</p></div></div>
  ...
  <div className="dashboard-grid-bottom">
   <div className="panel"><div className="panel-head"><div><span className="eyebrow">AKTIVITAS TERBARU</span><h2>Aktivitas Absensi Terbaru</h2><p>Data absensi terbaru yang masuk ke sistem.</p></div><button className="link-btn" onClick={()=>onNavigate('attendance')}>Lihat semua</button></div><AttendanceMini rows={recent}/></div>
...
function AttendanceModule({type,data,onRefresh,onExport}:{type:MenuKey;data:Absensi[];onRefresh:()=>void;onExport:()=>void}){const {t}=useTranslation();
 ...
 const del=async(id:string)=>{if(await appConfirm('Hapus data absensi ini?')){const {error}=await supabase.from('absensi').delete().eq('id',id);if(error)await appAlert(error.message);else onRefresh()}};
 return <Branch title="Absensi" desc="Rekap, input manual, peninjauan keterlambatan, lembur, dan selfie." items={items.map(([key,label,icon])=>({key,label,icon}))} tab={tab} setTab={setTab} action={tab==='summary'?'＋ Input Absensi': 'Export CSV'} onAction={tab==='summary'?()=>setOpen(true):onExport}>
  <div className="stat-grid three"><Stat title="Data" value={String(rows.length)} hint="Data ditampilkan" icon="calendar"/><Stat title="Hadir" value={String(rows.filter(a=>/hadir|tepat|terlambat/i.test(a.status||'')).length)} hint="Kehadiran" icon="check"/><Stat title="Perlu Ditinjau" value={String(rows.filter(a=>Number(a.lembur_menit||0)>0||Number(a.keterlambatan_menit||0)>0).length)} hint="Lembur / terlambat" icon="alert"/></div>
...
function HolidayModule(){
 ...
 const del=async(id:string)=>{if(await appConfirm('Hapus hari libur?')){const {error}=await supabase.from('hris_hari_libur').delete().eq('id',id);if(error)setMsg(error.message);else load()}};
...
function TalentModule({initial,employees}:{initial:MenuKey;employees:Karyawan[]}){
 ...
 const items=[['performance','Kinerja','arrow'],['kpi','KPI & Target','kpi'],['vacancies','Lowongan','recruitment'],['candidates','Kandidat','users'],['interviews','Wawancara','calendar']];
 return <Branch title="Talent" desc="Kinerja, KPI, recruitment, kandidat, dan interview dengan data persisten." items={items.map(([key,label,icon])=>({key,label,icon}))} tab={tab} setTab={setTab} action="＋ Tambah" onAction={()=>setModal(true)}>{<TalentTable tab={tab} rows={rows}/>} {modal&&<TalentForm tab={tab} employees={employees} onClose={()=>setModal(false)} onSaved={()=>{setModal(false);load()}}/>}</Branch>
}
...
function ReportCard({name,count,onClick}:{name:string;count:number;onClick:()=>void}){return <div className="report-card"><span>LAPORAN</span><h3>{name}</h3><b>{count}</b><p>data tersedia</p><button className="primary" onClick={onClick}>Export CSV</button></div>}
function Settings(){
