...
function safeImageHref(value: string) {
  ...
  if (/^(?:https?:\/\/|\/|data:image\/|blob:)/i.test(trimmed)) return escapeXml(trimmed);
  return '';
}
...
function CardArtwork({ employee, side, companyName, logoUrl, photoOverride }: { employee: Employee; side: 'front' | 'back'; companyName: string; logoUrl: string; photoOverride?: string }) {
  ...
  const photoSvg = photo
    ? `<image href="${safeImageHref(photo)}" x="68" y="152" width="190" height="236" preserveAspectRatio="xMidYMid slice"/>`
    : `<rect x="68" y="152" width="190" height="236" rx="18" fill="#eef2f7"/><text x="163" y="286" text-anchor="middle" font-size="62" font-weight="700" fill="#101a33">${escapeXml(initials(employee.nama))}</text>`;
  ...
  if (side === 'back') {
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
      ...
      <image href="${safeImageHref(logoUrl)}" x="54" y="27" width="58" height="58" preserveAspectRatio="xMidYMid meet"/>
      <text x="132" y="65" font-family="Arial" font-size="27" font-weight="700" fill="#fff">${escapeXml(companyName)}</text>
      ...
      <text x="54" y="350" font-family="Arial" font-size="25" font-weight="700" fill="#101a33">${escapeXml(id)}</text>
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 856 540">
    ...
    <image href="${safeImageHref(logoUrl)}" x="54" y="30" width="66" height="66" preserveAspectRatio="xMidYMid meet"/>
    <text x="140" y="64" font-family="Arial" font-size="28" font-weight="700" fill="#fff">${escapeXml(companyName)}</text>
...
export default function IDCardModule({ employees, companyName, logoUrl }: Props) {
  ...
  const cetakCards = async (ids: string[]) => {
    ...
    win.document.write(`
      <html>
        <head>
          <title>ID Card ${escapeXml(companyName)}</title>
          <style>
            @page {
              size: A4 portrait;
              margin: 0;
