...
export default function Employee360({employees, initialEmployeeId}:{employees:Employee[];initialEmployeeId?:string}){
 ...
 if(!employees.length)return <div className="panel empty-state"><h3>t('no_employees')</h3><p>t('add_employee_first_360')</p></div>;
 return <><div className="page-heading"><div><span className="group-title">t('people_360')</span><h1>t('employee_360')</h1><p>t('employee_360_desc')</p></div><select className="employee-picker" value={selected} onChange={e=>setSelected(e.target.value)}>{employees.map(x=><option key={x.id} value={x.id_karyawan}>{x.nama} — {x.id_karyawan}</option>)}</select></div>
 ...
 <div className="mini-kpi-row"><Stat label={t("attendance")} value={attendance.length}/><Stat label={t("leave")} value={leave.length}/><Stat label={t("overtime")} value={overtime.length}/><Stat label={t("payroll")} value={payroll.length}/><Stat label={t("documents")} value={docs.length}/></div>
 ...
 {tab==='overview'&&<div className="employee-overview"><div><h3>{t('profile')}</h3><dl><dt>{t('employee_id')}</dt><dd>{emp?.id_karyawan}</dd><dt>{t('position')}</dt><dd>{emp?.jabatan||'—'}</dd><dt>{t('level')}</dt><dd>{emp?.level_jabatan||'—'}</dd><dt>{t('department')}</dt><dd>{emp?.departemen||'—'}</dd><dt>{t('join_date')}</dt><dd>{emp?.tanggal_masuk||'—'}</dd><dt>{t('basic_salary')}</dt><dd>{money(Number(emp?.gaji_pokok||0))}</dd></dl></div><div><h3>{t('latest_payroll')}</h3>{payroll[0]?<><b className="big-money">{money(payroll[0].gaji_bersih)}</b><p>{t('period')} {payroll[0].periode} · {payroll[0].status}</p></>:<p>{t('no_payroll')}</p>}<h3>{t('recent_activity')}</h3>{history.slice(0,4).map(x=><p key={x.id}><b>{x.jenis}</b> · {x.ke_nilai||'—'} <small>{String(x.created_at||'').slice(0,10)}</small></p>)}</div></div>}
...
function Simple({rows,cols}:{rows:Row[];cols:string[]}){ const { t } = useTranslation();return <div className="table-wrap"><table><thead><tr>{cols.map(c=><th key={c}>{c.replaceAll('_',' ')}</th>)}</tr></thead><tbody>{rows.length?rows.map((r,i)=><tr key={r.id||i}>{cols.map(c=><td key={c}>{String(r[c]??'—')}</td>)}</tr>):<tr><td colSpan={cols.length} className="empty-cell">{t('no_data')}</td></tr>}</tbody></table></div>}
